...
load("additional_summaries.RData")
```
...
The table below summarises HH income. A total of `r sum(wealthindex_hh_tab$n>1, na.rm = TRUE)` respondents had equal incomes in at least two interview instances.
...
However, if we aggregate HH incomes by year, all the members in the households had different incomes (`equal_income = ` `r sum(wealthindex_tab$n>1, na.rm = TRUE)`).
