...
- **mechanistic way**
	- If the household, $h$, has service, $s$,  at the current time, $t$ i.e., $y_{hts} = 1$, then the odd of losing service in $t+1$ is $\mu_{hts} = \beta_{lose} - \eta_{hts}$. The switch probability is given by
\[
\pi_{ht} = \frac{1}{1 + \exp{(-\mu_{hts})}}
\]
	- Otherwise, if $y_{hts} = 0$, the odd of gaining service in the next year is $\mu'_{hts} = \beta_{gain} + \eta_{hts}$ with a switch probability of
\[
\pi_{ht} = \frac{1}{1 + \exp{(-\mu'_{hts})}}
\]

- **simplified (or statistical) way**

	- The odd of household $h$ at time $t$ gaining service $s$ is defined as
\[
\mu_{hts} = \eta_{hts} + \beta_{gain} + \beta_{add}\mu_{ht-1,s}\\
...
\]
	- $\mu_{hts}$, $t = 0$ can be chosen at random

For this task, we use the simplified approach since we established that the two approaches give the same results.

Thus for households $s$ at time $t$, the observed value for service $s$ is drawn from a binomial distribution i.e.,
\[
y_{hts} \sim \mathit{Bin}(1, \pi_{hts}).
\]

We simulated observed values for two services $(s = \{1,2\})$ for $5000$ households running for $100$ years and excluded the first $20$ years from the analysis (to take care of stationarity of the AR(1) process).
...
- $\beta_{0, hts}$ is the baseline odds of households gaining service 
- $\beta_{1,s}$ is the overall effect size of improved service in the previous year on status of service in the current year (odds of gaining service in the current year)
- $\beta_{2,s}$ is the overall effect size of household income (measure covariate, x) on the status of service 
- $\alpha_{0,hts}$ captures the offset of household odds of gaining service from the overall baseline (mean)
...
Figures below compares true parameter values against the estimates (lines) for $5000$ simulations. 
