...
The household services for the time period $t + 1$ are generated using _switch_ probabilities. Let $\beta_{lose}$ be the log odd of losing service and $\beta_{have}$ be the log odd of having service, switch probabilities are either defined in a:

- **mechanistic way**
	- If the household, $h$, has service, $s$,  at the current time, $t$ i.e., $y_{hts} = 1$, then the log odd of losing service in $t+1$ is $\mu_{hts} = \beta_{lose} - \eta_{hts}$. The switch probability is given by
\[
\pi_{ht} = \frac{1}{1 + \exp{(-\mu_{hts})}}
\]
	- Otherwise, if $y_{hts} = 0$, the log odd of having service in the next year is $\mu'_{hts} = \beta_{have} + \eta_{hts}$ with a switch probability of
\[
...
- **simplified (or statistical) way**
	...
	- The log odd of household $h$ at time $t+1$ having service $s$ is defined as
\[
\mu_{ht+1, s} = \eta_{ht+1, s} + \beta_{have} + \beta_{add}\mathbf{I}(y_{ht,s} = 1)\\
\beta_{add} = -(\beta_{have} + \beta_{lose}).
\]
...
betas_df2 <- (betas_df
	%>% mutate(parameter = ifelse(grepl("\\_M", coefs2), "xm"
		, ifelse(grepl("gain", coefs2), "Add", "statusP1"))
		, term = gsub("\\_.*", "", coefs2)
	)
)
...
dd <- (betas_df
	%>% mutate(var = ifelse(grepl("\\_M", coefs2), "xm"
		, ifelse(grepl("gain", coefs2), "Add", "statusP"))
		, term = gsub("\\_.*", "", coefs2)
		, probs = round(probs, 3)
	)
