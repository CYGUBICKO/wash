...
ordpred <- function(mod, n, modAns){
  ...
  if (inherits(mod, "glmerMod")){
	  v$fit <- ordTrans(v$fit, mod@beta, length(mod@beta))
	  v$lwr <- ordTrans(v$lwr, mod@beta, length(mod@beta))
	  v$upr <- ordTrans(v$upr, mod@beta, length(mod@beta))
  } else {
...
varpred <- function(mod, varname, frame, isolate=FALSE, isoValue=NULL, level=0.05, steps=101, dfspec=100, vv=NULL){
  ...
  if (isolate) {
	...
	# Steve: data.matrix in vc
  pse_var <- sqrt(diag(mmvar %*% tcrossprod(data.matrix(vc), mmvar)))
