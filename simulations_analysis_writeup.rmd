...
```{r}
nhh <- 10 # Number of hh to vizualize
...
year_specific_plot <- (ggplot(posterior_estimates
		%>% filter(grepl("^b\\[y[1-3]\\|\\(Intercept\\) years:", terms))
		, aes(terms, X50., ymin = X10., ymax = X90.)
	)
	+ geom_pointrange()
	+ coord_flip()
